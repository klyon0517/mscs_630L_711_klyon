...
<html  xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body>
    ...
    <script>
      ...
      var m = 9; // modulo
      var n = 3; // matrix size
      var matrix = [7, 5, 2, 0, 6, 4, 8, 2, 5];
      // Output 3 = 2
      
      // input.3a.1.in
      // var m = 27972; // modulo
      // var n = 1; // matrix size
      // var matrix = [-520989692];
      // input.3a.1.ans = 16780
      
      var x = 0;
      
      
      const row = n, col = n; 
      var A = Array(row).fill().map(() => Array(col));
       ...
       */
      function cofModDet(m, A) {
            
        var determinantMod = 1;
        var len = A.length;
        ...
        if (len == 2) {
          ...
          var det1 = (((A[0][0] % m) + m) % m) * (((A[1][1] % m) + m) % m);            
          var det2 = (((A[0][1] % m) + m) % m) * (((A[1][0] % m) + m) % m);
        ...
        for (var skipCol = 0; skipCol < len; ++skipCol) {
          
          const x = len - 1, y = len - 1; 
          var matrix = Array(x).fill().map(() => Array(y));
