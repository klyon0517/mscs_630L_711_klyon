<!--  BabylonAES
      ...
      * Filename: babylonAES_lab3_v4.php
      * Author: Kerry Lyon
      * Created: March 23, 2022

      * Converting and animating lab 3. Matching animation
  ...
  <body>
    ...
    <script>
      ...
      const createScene =  () => {
        ...
        // 16 bit strings
        function linStr(subLetter, dat4) {
          ...
          // for (var x = 0; x < len; x++) {
            ...
            for (var i = 0; i < 16; i++) {
              ...
              plainDynaTex.drawText(letter, null, null, font, "blue", "yellow", true);
              plainTextPlane.position = new BABYLON.Vector3(((i * 1.25) + (dat4 * 20)) - 8, 3, -1);
              
              //parameters - arrayOfMeshes, disposeSource, allow32BitsIndices, meshSubclass, subdivideWithSubMeshes, multiMultiMaterial
              // var mesh = []
              // mesh += BABYLON.Mesh.MergeMeshes([plainTextPlane], true, true, undefined, false, true);
              ...
              visFrames.push({
                frame: (dat4 * 3) * frameRate / 2,
                value: 0,
              });
              
              visFrames.push({
                frame: (2 + (dat4 * 3)) * frameRate / 2,
                value: 0,
              });
              ...
              visFrames.push({
                frame: ((len * 6) + 7) * frameRate / 2,
                value: 0,
              });

              visibleTog.setKeys(visFrames);
              
              scene.beginDirectAnimation(plainTextPlane, [visibleTog], 0, ((len * 6) + 7) * frameRate / 2, false);
            ...
            visFrames.push({
              frame: 13 * frameRate / 2,
              value: 0,
            });

            visibleTog.setKeys(visFrames);
            
            scene.beginDirectAnimation(mesh, [visibleTog], 0, 13 * frameRate / 2, false); */
        ...
        for (var i = 0; i < len; i++) {
          ...
          for (var x = 0; x < matrix.length; x++) {
            ...
            for (var y = 0; y < matrix[x].length; y++) {
              ...
              matrixFrames.push({
                frame: (11 + (tac + ((1 + (i * 16))))) * frameRate / 2,
                value: 0,
              });
        ...
        function hexPlanes(matrix, dat4) {
          ...
          for (var x = 0; x < 4; x++) {
            ...
            for (var y = 0; y < 4; y++) {
              ...
              hexFrames.push({
                frame: (len + 9) * frameRate / 2,
                value: 1,
              });

              hexMatrix.setKeys(hexFrames);
              
              scene.beginDirectAnimation(hexPlane, [hexMatrix], 0, (len + 9) * frameRate / 2, false);
